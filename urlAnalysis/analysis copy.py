import pandas as pd
import joblib
import numpy as np
from urlAnalysis.pars import get_url_features

class URLAnalyzer:
    def __init__(self, rf_model_path, rf_multiclass_path):
        """
        URL analiz sistemini başlatır ve modelleri yükler

        Args:
            rf_model_path: Zararlı mı değil mi modeli (binary)
            rf_multiclass_path: Zararlı tür tespiti modeli (multiclass)
        """
        self.rf_model = joblib.load(rf_model_path)
        self.rf_multiclass = joblib.load(rf_multiclass_path)

        # Zararlı tür sınıfları (eğitimde kullanılan sırayla eşleşmeli)
        self.malware_types = {
            0: "Defacement",
            1: "malware",
            2: "phishing",
            3: "spam"
        }

    def analyze_url(self, url):
        """
        Kullanıcıdan aldığı URL'yi analiz eder ve sonucu döndürür

        Args:
            url: Analiz edilecek URL

        Returns:
            dict: Analiz sonuçları
        """
        try:
            # URL'den özellikleri çıkar
            features = get_url_features(url, 0)

            # DataFrame oluştur
            features_df = pd.DataFrame([features])
            features_df = features_df.drop(columns=['URL_Type_obf_Type']).select_dtypes(include=[np.number])

            # Zararlı mı? - Binary tahmin
            rf_prediction_proba = self.rf_model.predict_proba(features_df)[0]
            is_malicious = rf_prediction_proba[1] > 0.5

            result = {
                "url": url,
                "is_malicious": bool(is_malicious),
                "benign_probability": float(rf_prediction_proba[0]),
                "malicious_probability": float(rf_prediction_proba[1])
            }

            # Zararlıysa, türünü tahmin et
            if is_malicious:
                predicted_class = self.rf_multiclass.predict(features_df)[0]
                malware_type = self.malware_types.get(int(predicted_class), "Unknown")
                result["malware_type"] = malware_type

            return result

        except Exception as e:
            return {
                "error": f"URL analiz edilirken hata oluştu: {str(e)}",
                "url": url,
                "is_malicious": None
            }

if __name__ == "__main__":
    # Model dosya yolları
    rf_model_path = "first-stage-pkl/random_forest_binary.pkl"
    rf_multiclass_path = "second-stage-pkl/rf_malicious_type_model.pkl"

    # Analiz sınıfını başlat
    analyzer = URLAnalyzer(rf_model_path, rf_multiclass_path)

    while True:
        url = input("\nAnaliz edilecek URL'yi girin (Çıkış için Enter'a basın): ").strip()
        if not url:
            print("Çıkış yapılıyor...")
            break

        result = analyzer.analyze_url(url)
        print(f"\nURL: {result['url']}")
        if "error" in result:
            print(f"Hata: {result['error']}")
        else:
            print(f"Zararlı mı?: {result['is_malicious']}")
            print(f"Zararsız olma olasılığı: {result['benign_probability']:.2%}")
            print(f"Zararlı olma olasılığı: {result['malicious_probability']:.2%}")
            if result['is_malicious']:
                print(f"Zararlı Yazılım Türü: {result['malware_type']}")
