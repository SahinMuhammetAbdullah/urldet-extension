...
def get_tld_length(url):
    """
    Bir URL'nin TLD'sinin uzunluğunu hesaplar (ilk noktayı saymaz ve eksik ayrıştırmayı düzeltir).
    """
    # tldextract ile domain ayrıştırma
    extracted = tldextract.extract(url)
    tld = extracted.suffix  # TLD kısmını al

    # Eğer tldextract doğru bir TLD döndürmezse, domainin son kısmını al
    if not tld:
        domain = extracted.domain
        tld = domain.split(".")[-1]  # Son noktadan sonraki kısmı al
...
def is_executable(path):
    executable_extensions = {
        ".exe", ".bat", ".cmd", ".dll", ".com", ".msi", ".sys", ".scr",
        ".vbs", ".js", ".sh", ".run", ".bin", ".pl", ".cgi", ".py",
        ".php", ".asp", ".jsp", ".rb", ".so", ".jar", ".war", ".class",
        ".ps1", ".bash", ".ksh", ".zsh", ".csh", ".out", ".o", 
        ".lib", ".a", ".dylib", ".apk", ".deb", ".rpm", ".tar.gz",
    }
    # Dosya uzantısını kontrol et
    file_extension = path.split(".")[-1] if "." in path else ""
    return 1 if f".{file_extension}" in executable_extensions else 0
